...
pmfmax=120;factor=pmfmax/abs(pmfmax)
pmfmin=0
# Read 2D fes file
...
x=df[df.columns[0]].to_numpy()#Este -1, depende de la definición inicial, igual en tu caso se puede borrar
...
for i in range(1,n1Dplots):
    axs[0].plot(xd1[i]/2,yd1[i])#O -1 depende da tua definición de coordenada x.
    axs[3].plot(yd2[i], xd2[i]/2)
axs[1].axis('off')
axs[3].yaxis.tick_right()
levels=list(range(0, pmfmax, 10))#Niveles a representar
levels=np.sort(levels)
cpf = axs[2].contourf(xi/2, yi/2, zi,100,vmin=min(levels),vmax=max(levels),extend='max',cmap=cm.rainbow.reversed(),ticks=levels)
cpf.changed()
...
cp = axs[2].contour(xi/2, yi/2, zi, levels, colors='black', linestyles='dashed')
...
axs[2].set_xlabel(r'$\mathbf{ETP-ETP \,\,Contacts\,\,}$', fontsize=20)
axs[2].set_ylabel(r'$\mathbf{ Membrane-ETP\,\, Contacts\,\,}$', fontsize=20)
